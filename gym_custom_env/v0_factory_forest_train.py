...
import v0_factory_forest_env

import numpy as np
import pandas as pd

def equallySpacedWeights(n):
    weights = []
    interval_between = 1 / (n-1)

    for i in range(n):  # Step 2
        start_value = interval_between * i 
        end_value = 1.0 - interval_between * i 
        weight = np.array([start_value, end_value])
...
if __name__ == '__main__':
    ...
    env = mo_gym.make('factory-forest-env-v0')
    env = MORecordEpisodeStatistics(env, gamma=GAMMA)  # wrapper for recording statistics
    eval_env = mo_gym.make('factory-forest-env-v0') # environment used for evaluation

    # agent = PGMORL(
    #     env_id='factory-forest-env-v0',
    ...
    #     print("Run Number: ", i)
    #     env = mo_gym.make('factory-forest-env-v0', render_mode='human')
    #     obs, _ = env.reset()
    #     terminated = False
    ...
    agent.train(total_timesteps=10000, eval_env=eval_env, ref_point=ref_point)
    weights = equallySpacedWeights(6)
    for i in range(6):
        print("Run Number: ", i)
        env = mo_gym.make('factory-forest-env-v0', render_mode='human')
        obs, _ = env.reset()
        terminated = False
        while(not terminated):
            action = agent.eval(obs, weights[i])
            obs, reward, terminated, _, _ = env.step(action)
